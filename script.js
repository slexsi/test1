...
// ----------------- Gameplay Additions -----------------
const keyMap = { 'a':0, 's':1, 'k':2, 'l':3 }; // 4-lane keys
const laneKeys = Object.keys(keyMap);
const hitLine = canvas.height - 60;
const hitWindow = 30; // px
let score = 0;
let combo = 0;
let feedback = '';
let feedbackTimer = 0;
...
window.addEventListener('keydown', e => {
  ...
  if (laneKeys.includes(k)) keys[k] = true;
});
window.addEventListener('keyup', e => {
  const k = e.key.toLowerCase();
  if (laneKeys.includes(k)) keys[k] = false;
});

// Modify spawnVisualNote to map pitch to 4-lane game
function spawnVisualNote(lane) {
  // Map the original 8 lanes into 4-lane keys
  const mappedLane = Math.floor(lane / 2); // 0..7 -> 0..3
  notes.push({ lane: mappedLane, y: -20, speed: 2 + Math.random()*1.5, hit:false });
}

// Extend draw() to draw hit line and handle scoring
function draw() {
  ctx.clearRect(0,0,canvas.width,canvas.height);
  
  // draw lanes
  for(let i=0;i<4;i++){
    ctx.fillStyle = keys[laneKeys[i]] ? '#0f0' : '#111';
    ctx.fillRect(i*laneW,0,laneW-2,canvas.height);
  }
  
  // draw hit line
  ctx.fillStyle = '#ff0';
  ctx.fillRect(0, hitLine, canvas.width, 5);
  
  // draw notes
  notes.forEach(n=>{
    ctx.fillStyle = n.hit ? '#555' : ['#e74c3c','#f1c40f','#2ecc71','#3498db'][n.lane%4];
    ctx.fillRect(n.lane*laneW + 6, n.y, laneW-12, 16);
    
    // check hits
    const keyPressed = keys[laneKeys[n.lane]];
    if(!n.hit && keyPressed && Math.abs(n.y - hitLine)<hitWindow){
      n.hit = true;
      const delta = Math.abs(n.y - hitLine);
      if(delta < 10){ score += 300; feedback='PERFECT'; }
      else if(delta < 20){ score += 150; feedback='GOOD'; }
      else{ score += 50; feedback='OK'; }
      combo++;
      feedbackTimer = 30;
      updateScore();
    }
  ...
  if(feedbackTimer>0){
    ctx.fillStyle = '#fff';
    ctx.font = '24px Arial';
    ctx.fillText(feedback, canvas.width/2 - 40, hitLine - 30);
    feedbackTimer--;
  }
  
  // remove hit/offscreen notes
